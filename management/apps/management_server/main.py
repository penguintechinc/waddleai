...
from shared.auth.penguin_auth import create_oidc_provider, issue_token, verify_token
from shared.auth.rbac import Permission, RBACManager, Role
...
async def dashboard(request: FastAPIRequest):
    ...
    try:
        ...
        return templates.TemplateResponse(
            "dashboard.html", {"request": request, "user": user_context, "data": dashboard_data}
        )
...
async def _get_dashboard_data(user_context) -> Dict[str, Any]:
    ...
    if user_context.role == Role.ADMIN:
        ...
        total_api_keys = mgmt_server.db(mgmt_server.db.api_keys.enabled is True).count()
    ...
    # Organization manager dashboard
        ...
        org_users = mgmt_server.db(mgmt_server.db.users.organization_id == user_context.organization_id).count()
        ...
        data.update(
            {
                "organization_stats": {"total_users": org_users, "monthly_token_usage": total_tokens},
    ...
    # Reporter dashboard
        ...
        data.update({"usage_analytics": [dict(usage) for usage in org_usage]})
    ...
    # User dashboard
        ...
        user_api_keys = mgmt_server.db(mgmt_server.db.api_keys.user_id == user_context.user_id).select()

        data.update(
            {"personal_usage": [dict(usage) for usage in user_usage], "api_keys": [dict(key) for key in user_api_keys]}
        )
...
async def routing_config(request: FastAPIRequest):
    ...
    try:
        ...
        try:
            ...
            routing_data = {"instructions": "Error loading routing instructions", "routing_llm": "unknown"}

        return templates.TemplateResponse(
            "routing_config.html", {"request": request, "user": user_context, "data": routing_data}
        )
...
async def providers(request: FastAPIRequest):
    ...
    try:
        ...
        return templates.TemplateResponse(
            "providers.html", {"request": request, "user": user_context, "data": providers_data}
        )
...
async def memory_config(request: FastAPIRequest):
    ...
    try:
        ...
        if not (
            mgmt_server.rbac.check_permission(user_context, Permission.SYSTEM_CONFIG)
            or mgmt_server.rbac.check_permission(user_context, Permission.REPORT_VIEW)
        ):
            raise HTTPException(status_code=403, detail="Insufficient permissions")

        # Get memory stats
        memory_data = {"total_conversations": 0, "storage_size_mb": 0, "backend": "mem0", "status": "connected"}

        return templates.TemplateResponse(
            "memory_config.html", {"request": request, "user": user_context, "data": memory_data}
        )
...
async def redis_config(request: FastAPIRequest):
    ...
    try:
        ...
        redis_data = {"connected": False, "url": mgmt_server.config["redis_url"], "version": "unknown"}
        ...
        return templates.TemplateResponse(
            "redis_config.html", {"request": request, "user": user_context, "data": redis_data}
        )
...
async def mcp_management(request: FastAPIRequest):
    ...
    try:
        ...
        return templates.TemplateResponse(
            "mcp_management.html", {"request": request, "user": user_context, "data": mcp_data}
        )
...
async def api_keys_enhanced(request: FastAPIRequest):
    ...
    try:
        ...
        if mgmt_server.rbac.check_permission(user_context, Permission.SYSTEM_CONFIG):
            ...
            keys = mgmt_server.db(mgmt_server.db.api_keys.organization_id == user_context.organization_id).select()
        else:
            keys = mgmt_server.db(mgmt_server.db.api_keys.user_id == user_context.user_id).select()
        ...
        return templates.TemplateResponse(
            "api_keys_enhanced.html", {"request": request, "user": user_context, "data": keys_data}
        )
...
async def analytics_enhanced(request: FastAPIRequest):
    ...
    try:
        ...
        return templates.TemplateResponse(
            "analytics_enhanced.html", {"request": request, "user": user_context, "data": analytics_data}
        )
...
async def performance_config(request: FastAPIRequest):
    ...
    try:
        ...
        xdp_data = {
            ...
            "performance": {"drop_rate": 0.0, "throughput_mbps": 0.0},
            "message": "XDP not enabled. Set ENABLE_XDP=true and run as root to enable.",
        }

        return templates.TemplateResponse(
            "performance_config.html", {"request": request, "user": user_context, "data": xdp_data}
        )
...
async def dashboard_enhanced(request: FastAPIRequest):
    ...
    try:
        ...
        return templates.TemplateResponse(
            "dashboard_enhanced.html", {"request": request, "user": user_context, "data": dashboard_data}
        )
...
async def create_organization(request: FastAPIRequest, user_context=Depends(get_current_user)):
...
async def list_users(user_context=Depends(get_current_user)):
    ...
    if mgmt_server.rbac.check_permission(user_context, Permission.SYSTEM_CONFIG):
        ...
        users = mgmt_server.db(mgmt_server.db.users.organization_id == user_context.organization_id).select()

    return {"users": [dict(user) for user in users]}


@app.post("/api/users")
async def create_user(request: FastAPIRequest, user_context=Depends(get_current_user)):
    """Create user"""
    # Check permissions
    if not (
        mgmt_server.rbac.check_permission(user_context, Permission.SYSTEM_CONFIG)
        or mgmt_server.rbac.check_permission(user_context, Permission.RESOURCE_MANAGE)
    ):
        raise HTTPException(status_code=403, detail="Insufficient permissions")
...
async def create_connection_link(request: FastAPIRequest, user_context=Depends(get_current_user)):
...
async def create_api_key(request: FastAPIRequest, user_context=Depends(get_current_user)):
    """Create API key"""
    try:
        body = await request.json()
        ...
        if target_user_id != user_context.user_id:
            if not (
                ...
                or (
                    ...
                    and target_org_id == user_context.organization_id
                )
            ):
                raise HTTPException(status_code=403, detail="Insufficient permissions")
    ...
    try:
        ...
        if key.user_id != user_context.user_id:
            if not (
                ...
                or (
                    mgmt_server.rbac.check_permission(user_context, Permission.RESOURCE_MANAGE)
                    and key.organization_id == user_context.organization_id
                )
            ):
                raise HTTPException(status_code=403, detail="Insufficient permissions")
...
@app.get("/api/usage")
async def get_usage_stats(user_context=Depends(get_current_user), days: int = 30):
...
async def ollama_pull_model(request: FastAPIRequest, user_context=Depends(get_current_user)):
...
async def ollama_remove_model(model_name: str, user_context=Depends(get_current_user)):
...
async def set_routing_instructions(request: FastAPIRequest, user_context=Depends(get_current_user)):
    ...
    try:
        ...
        return {"status": "success", "instructions_length": len(instructions), "routing_llm": routing_llm}
    except HTTPException:
        raise
    except Exception as e:
...
async def test_routing_decision(request: FastAPIRequest, user_context=Depends(get_current_user)):
...
async def search_conversations(
    ...
    try:
        ...
        if user_id and user_id != user_context.user_id:
            if not mgmt_server.rbac.check_permission(user_context, Permission.SYSTEM_CONFIG):
                raise HTTPException(
                    status_code=403, detail="Admin permission required to view other users' conversations"
                )
        ...
        return {"query": query, "count": len(conversations), "conversations": conversations}
...
async def get_memory_stats(user_context=Depends(get_current_user)):
    ...
    try:
        ...
        stats = {
            ...
            "models_used": {"claude-3-sonnet": 450, "gpt-4": 320, "llama-70b": 477},
        }
...
async def toggle_xdp(request: FastAPIRequest, user_context=Depends(get_current_user)):
    ...
    try:
        ...
        return {
            ...
            "message": f"XDP acceleration {status_msg}. Restart proxy server to apply changes.",
        }
