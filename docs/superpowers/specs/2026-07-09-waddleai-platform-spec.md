...
**Not yet scoped**: model selection (local vs. hosted, OSI/non-PRC constraints per §2.1/§2.2 apply the same as everywhere else), fleet-backend integration (§10.1), storage/serving of generated artifacts, licensing-tier placement (§2.4), and delivery timing are all open. This section exists to record product direction and the safety-layer non-negotiable above it — a full spec pass (data model, migration, acceptance criteria, release wave) is required before this becomes buildable work.
