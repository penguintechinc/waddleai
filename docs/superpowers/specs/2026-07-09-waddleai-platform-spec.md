...
- **Admin-gated.** Enabling requires a user holding `Role.ADMIN` (`shared/auth/rbac.py`). A Resource Manager, Reporter, or ordinary user MUST NOT be able to accept this risk on the deployment's behalf, whatever model-configuration rights they otherwise hold — accepting a supply-chain risk is a different act from configuring a model, and the permission check is on the acceptance, not on the model edit that follows it. In a multi-tenant deployment the acceptance is scoped to the org the admin administers and does not carry to any other org.
