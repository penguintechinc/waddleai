...
|--------|------|----------------|
| Create | `services/management/alembic/versions/010_security_v2.py` | Migration 010: `security_policies`, `security_bypass_grants`, extend `content_filter_audit_log`, fold `content_filter_config` → scoped policies |
| Create | `tests/unit/management/test_migration_010.py` | Round-trip + downgrade on a seeded `content_filter_config`/`content_filter_rules` snapshot |
...
- [ ] **Step 4: Run tests, verify pass** — `python3 -m pytest tests/unit/management/test_migration_010.py -v --no-cov`; `alembic -c services/management/alembic.ini heads` shows single head `010_…`.

- [ ] **Step 5: Commit**
  ```bash
  git add services/management/alembic/versions/010_security_v2.py tests/unit/management/test_migration_010.py services/management/app/models_sqlalchemy.py
  git commit -m "feat(db): migration 010 — security_policies, bypass grants, extended audit log; fold content_filter_config" \
...
| §13.1 migration 010 round-trip + downgrade | 1 |
