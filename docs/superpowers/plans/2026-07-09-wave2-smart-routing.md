...
|--------|------|----------------|
| Create | `services/management/alembic/versions/009_routing_engine.py` | Migration 009: `model_configs`, `model_aliases`, `routing_rules_v2`, `routing_policies`, `routing_decision_traces`; evolve `routing_matrix` → `model_assignments` (add `tool_type`, `escalation_model`, `scope`/`scope_ref`); data-migrate hardcoded dict + `routing_matrix` rows; seed internal-function assignment rows |
...
| Create | `tests/unit/management/test_migration_009.py` | Round-trip + downgrade + data-migration + seed assertions on a seeded snapshot |
...
### Task 1: Migration 009 + ORM models + data migration + internal-function seed

Down-revision `008_cache_and_proxy_memory` (per §13.1 sequence; if 008 is not yet merged at build time, rebase `down_revision` to the actual head — see judgment note). Adds `model_configs` (seeded from the hardcoded `request_router` dict), `model_aliases`, `routing_rules_v2(priority, match jsonb, action jsonb)`, `routing_policies` (all §7.3 columns), `routing_decision_traces` (§7.4 durable corpus); evolves `routing_matrix` → `model_assignments` adding `tool_type`, `escalation_model`, `scope enum(global|org)`, `scope_ref`. Data-migrates existing `routing_matrix` rows into `model_assignments` and seeds the internal-function rows (`security-audit → shieldgemma:2b`, `routing-classifier → gemma3:1b`, `embeddings → nomic-embed-text`, `docs-fetch`, `summarize`) referencing `model_registry` (007) per the §2.3 dual-default pattern. Round-trip + downgrade tested (house rule).

**Files:** Create `services/management/alembic/versions/009_routing_engine.py`, `tests/unit/management/test_migration_009.py`. Modify `services/management/app/models_sqlalchemy.py`.

- [ ] **Step 1: Write failing round-trip test** — on a SQLite/seeded snapshot with sample `routing_matrix` rows + a seeded `model_registry`: `upgrade` → `model_assignments` exists carrying migrated rows with `tool_type`/`escalation_model`/`scope='global'`; `model_configs`/`model_aliases`/`routing_rules_v2`/`routing_policies`/`routing_decision_traces` exist; `model_configs` seeded from the hardcoded dict (assert `gpt-4`, `claude-3-*`, `llama3` rows); internal-function assignment rows present and reference registry entries; `downgrade` → 008 shape (document that `model_assignments` folds back to `routing_matrix`). Run → fails (no 009).
- [ ] **Step 2: Run test, verify it fails** — `python3 -m pytest tests/unit/management/test_migration_009.py -v --no-cov`.
- [ ] **Step 3: Implement migration 009** — guarded DDL; `op.rename_table`/`op.add_column` for `routing_matrix`→`model_assignments`; INSERT-SELECT data migration; `op.bulk_insert` seeds for `model_configs` (from the extracted dict) and internal-function assignments; complete `downgrade()`.
...
- [ ] **Step 5: Run tests, verify pass** — `alembic -c services/management/alembic.ini heads` shows a single head `009_...`.
- [ ] **Step 6: Commit**
  ```bash
  git add services/management/alembic/versions/009_routing_engine.py tests/unit/management/test_migration_009.py services/management/app/models_sqlalchemy.py
  git commit -m "feat(db): migration 009 — routing engine tables, routing_matrix→model_assignments, internal-function seed" \
...
| §13.1 migration 009 (routing tables + evolve routing_matrix) round-trip + downgrade | 1 |
