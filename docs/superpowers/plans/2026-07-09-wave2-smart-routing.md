...
- [ ] **Step 5: Commit**
  ...
  git commit -m "feat(routing): cascade stage 2 — classifier (stubbed unit, real-model nightly) + cascade orchestrator" \
             -m "Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>"
  ```
...
- [ ] **Step 4: Classifier recorded-output fixtures** — stubbed in the unit tier; a `@pytest.mark.gpu` real-`gemma4:e2b` job runs nightly.
...
| §7.2 stage 2 classifier: `tool_type`+`{complexity,domain,needs_reasoning}`, `gemma4:e2b` (Apache-2.0, no dual-default alt required) | 7 |
