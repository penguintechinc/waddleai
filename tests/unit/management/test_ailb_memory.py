...
def test_set_memory_config_missing_org_id(client, auth_headers):
    ...
    resp = client.post("/api/v1/ailb/memory-config", json={"enabled": True}, headers=auth_headers)
    assert resp.status_code == 400
    data = resp.get_json()
    assert "error" in data
    assert "organization_id" in data["error"]
...
def test_set_memory_config_requires_auth(client):
    ...
    resp = client.post("/api/v1/ailb/memory-config", json={"organization_id": 1, "enabled": True})
    assert resp.status_code == 401
...
def test_set_memory_config_db_error_returns_500(client, app_mock_db, auth_headers):
    ...
    resp = client.post("/api/v1/ailb/memory-config", json={"organization_id": 1, "enabled": True}, headers=auth_headers)
    assert resp.status_code == 500
...
def test_set_rag_config_missing_org_id(client, auth_headers):
    ...
    resp = client.post("/api/v1/ailb/rag-config", json={"enabled": True}, headers=auth_headers)
    assert resp.status_code == 400
...
def test_set_embedding_config_requires_auth(client):
    ...
    resp = client.post("/api/v1/ailb/embedding-config", json={"backend": "ollama"})
    assert resp.status_code == 401
...
def test_set_embedding_config_requires_admin(client, user_auth_headers):
    ...
    resp = client.post("/api/v1/ailb/embedding-config", json={"backend": "ollama"}, headers=user_auth_headers)
    assert resp.status_code == 403
