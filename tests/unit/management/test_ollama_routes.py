...
from tests.unit.management.route_conftest import make_mock_org, make_mock_user, make_mock_key
...
def test_update_ollama_deployment_success(client, app_mock_db, auth_headers):
    ...
    payload = {
        'name': 'new-name',
        'endpoint_url': 'http://new-endpoint:11434'
    }
...
def test_remove_ollama_model_success(client, app_mock_db, auth_headers):
    ...
    resp = client.delete(
        '/api/v1/ollama/deployments/1/models/llama3.2',
        headers=auth_headers
    )
...
def test_remove_ollama_model_not_found_deployment(client, app_mock_db, auth_headers):
    ...
    resp = client.delete(
        '/api/v1/ollama/deployments/999/models/llama3.2',
        headers=auth_headers
    )
...
def test_remove_ollama_model_not_found_model(client, app_mock_db, auth_headers):
    ...
    app_mock_db.return_value.select.side_effect = [
        make_select_result([dep]),
        make_select_result([])  # Model not found
    ]

    resp = client.delete(
        '/api/v1/ollama/deployments/1/models/nonexistent',
        headers=auth_headers
    )
