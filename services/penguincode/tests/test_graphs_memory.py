"""Memory-graph extractor (T13): LLM triples -> GraphNode/GraphEdge writes.
...
construction, scope-stamped writes, `source_metadata` scope-stamp reuse) is
fully exercised without either live dependency. One optional live test at
the bottom writes through a real `PostgresGraphStore` against
`TEST_DATABASE_URL`, mirroring `tests/test_graphs_knowledge.py`'s
skip-with-reason pattern -- the LLM stays mocked even there; only the store
write is live.

# regression: penguincode-knowledge-platform (T13 -- memory graph)
"""
...
from penguincode_cli.graphs.memory import DEFAULT_VISIBILITY, extract_memory_graph
...
requires_postgres = pytest.mark.skipif(
    ...
    reason="TEST_DATABASE_URL is not set -- live-Postgres memory-graph tests are CI-pending (T16)",
)
...
class TestFlagGating:
    async def test_flag_off_skips_llm_and_writes(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr("penguincode_cli.graphs.memory.is_enabled", lambda *a, **kw: False)
        client = _mock_ollama_client(TRIPLES_JSON)
        store = _fake_graph_store()

        result = await extract_memory_graph(
            _ctx(), "Justin's dog Poppy went to the vet", ollama_client=client, graph_store=store
        )
...
class TestHappyPath:
    async def test_produces_expected_nodes_and_edges(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        result = await extract_memory_graph(
            ctx,
            "Justin owns Poppy and works at PenguinTech",
        ...
        assert edge_tuples == {
            ("person", "Justin", "owns", "pet", "Poppy"),
            ("person", "Justin", "works_at", "organization", "PenguinTech"),
        }
    ...
    async def test_empty_props_on_nodes_and_edges(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr("penguincode_cli.graphs.memory.is_enabled", lambda *a, **kw: True)
        client = _mock_ollama_client(TRIPLES_JSON)
        store = _fake_graph_store()

        result = await extract_memory_graph(_ctx(), "text", ollama_client=client, graph_store=store)

        assert all(n.props == {} for n in result.nodes)
        assert all(e.props == {} for e in result.edges)

    async def test_relation_is_normalized(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr("penguincode_cli.graphs.memory.is_enabled", lambda *a, **kw: True)
        raw = json.dumps(
            {"triples": [{"subject": "A", "relation": "Is Friends With", "object": "B"}]}
        )
        ...
        assert result.edges[0].rel_type == "is_friends_with"
    ...
    async def test_empty_content_short_circuits(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr("penguincode_cli.graphs.memory.is_enabled", lambda *a, **kw: True)
        client = _mock_ollama_client(TRIPLES_JSON)
        store = _fake_graph_store()

        result = await extract_memory_graph(_ctx(), "   ", ollama_client=client, graph_store=store)
...
# ---------------------------------------------------------------------------
# source_metadata: T8's exact scope stamp overrides the keyword defaults.
# ---------------------------------------------------------------------------


class TestSourceMetadataScopeReuse:
    async def test_source_metadata_visibility_and_team_id_override_defaults(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        monkeypatch.setattr("penguincode_cli.graphs.memory.is_enabled", lambda *a, **kw: True)
        client = _mock_ollama_client(TRIPLES_JSON)
        store = _fake_graph_store()
        ctx = _ctx(team_ids=("team-a",))
        # T8's `_scope_metadata()` shape: tenant_id/org_id/team_id/owner_user_id/visibility.
        source_metadata = {
    ...
    async def test_source_metadata_tenant_visibility_clears_team_id(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        monkeypatch.setattr("penguincode_cli.graphs.memory.is_enabled", lambda *a, **kw: True)
        client = _mock_ollama_client(TRIPLES_JSON)
        ...
        # Pass an explicit team_id keyword that source_metadata must override.
        await extract_memory_graph(
            ctx,
            "text",
            visibility="team",
            team_id="should-be-ignored",
            source_metadata=source_metadata,
            ollama_client=client,
            graph_store=store,
        )
        ...
        _, kwargs = store.upsert_nodes.call_args
        assert kwargs["visibility"] == "tenant"
        assert kwargs["team_id"] is None

    async def test_missing_source_metadata_falls_back_to_keyword_defaults(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        monkeypatch.setattr("penguincode_cli.graphs.memory.is_enabled", lambda *a, **kw: True)
        client = _mock_ollama_client(TRIPLES_JSON)
        store = _fake_graph_store()
        ctx = _ctx(team_ids=("team-b",))
    ...
    async def test_source_metadata_without_visibility_key_is_ignored(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        monkeypatch.setattr("penguincode_cli.graphs.memory.is_enabled", lambda *a, **kw: True)
        client = _mock_ollama_client(TRIPLES_JSON)
        ...
        await extract_memory_graph(
            ctx,
            "text",
            source_metadata={"tenant_id": ctx.tenant_id},  # no "visibility" key
            ollama_client=client,
            graph_store=store,
        )
...
class TestDefaultOllamaClientConstruction:
    async def test_builds_and_closes_own_client_when_none_injected(
        ...
        monkeypatch.setattr("penguincode_cli.graphs.memory.OllamaClient", _factory)
        store = _fake_graph_store()

        result = await extract_memory_graph(_ctx(), "text", graph_store=store)
...
class TestLiveGraphStoreWrite:
    async def test_extract_memory_graph_writes_through_real_store(
        ...
        subgraph = store.subgraph(ctx, "memory", ["Justin"], depth=1)
        assert {n.key for n in subgraph.nodes} == {"Justin", "Poppy", "PenguinTech"}
        ...
        isolated = store.subgraph(ctx, "memory", ["Justin"], depth=1)
