...
def rotate_key(key_id):
    ...
    db(db.virtual_keys.id == key_id).update(
        key_hash=bcrypt.hash(new_api_key),
        key_prefix=key_prefix,
        ailb_sync_status='pending'
    )
    db.commit()
...
def sync_key(key_id):
    ...
    return jsonify({
        'key_id': key_id,
        'ailb_sync_status': 'synced',
        'message': 'Key synced to AILB successfully'
    })
...
def get_key_usage(key_id):
    ...
    usage_records = db(
        (db.token_usage.virtual_key_id == key_id) &
        (db.token_usage.date >= start_date)
