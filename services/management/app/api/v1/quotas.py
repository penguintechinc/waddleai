...
def set_key_quota(key_id):
    ...
    return jsonify({
        'key_id': key_id,
        'key_name': key.name,
        'message': 'Key quota updated successfully. Re-sync to AILB required.'
    })
...
def get_quota_status(entity_id):
    ...
    if entity_type == 'key':
        ...
        # Get usage
        daily_usage = db(
            (db.token_usage.virtual_key_id == entity_id) &
        ...
        return jsonify({
            ...
            'quotas': {
                ...
                'monthly': {
                    ...
                    'percentage': (monthly_cost / key.budget_limit_monthly * 100) if key.budget_limit_monthly else 0
                },
                'rate_limits': {
                    'tpm_limit': key.tpm_limit,
                    'rpm_limit': key.rpm_limit
        ...
        daily_usage = db(
            (db.token_usage.user_id == entity_id) &
            (db.token_usage.date == today)
        ).select()
        ...
        return jsonify({
            ...
            'quotas': {
                ...
                'monthly': {
                    ...
                    'percentage': (monthly_tokens / user.token_quota_monthly * 100) if user.token_quota_monthly else 0
                }
            }
        })
        ...
        daily_usage = db(
            (db.token_usage.organization_id == entity_id) &
            (db.token_usage.date == today)
        ).select()
