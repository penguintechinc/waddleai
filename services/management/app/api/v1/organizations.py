...
def get_organization(org_id):
    ...
    # Get statistics
    user_count = db(db.users.organization_id == org_id).count()
    key_count = db(db.virtual_keys.organization_id == org_id).count()
...
def create_organization():
    ...
    existing = db(db.organizations.name == data["name"]).select().first()
    if existing:
...
def update_organization(org_id):
    ...
    if "name" in data:
        ...
        existing = db((db.organizations.name == data["name"]) & (db.organizations.id != org_id)).select().first()
        if existing:
            return jsonify({"error": "Organization name already exists"}), 409
...
def get_organization_usage(org_id):
    ...
    daily_usage = db((db.token_usage.organization_id == org_id) & (db.token_usage.date == today)).select()

    monthly_usage = db((db.token_usage.organization_id == org_id) & (db.token_usage.date >= month_start)).select()
