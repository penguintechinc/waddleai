...
def get_usage_summary():
    ...
    daily_records = db(daily_query).select()
    monthly_records = db(monthly_query).select()
...
def get_usage_by_model():
    ...
    if user_role == 'admin':
        ...
        base_query = (db.usage_logs.timestamp >= datetime.combine(start_date, datetime.min.time())) & (db.usage_logs.organization_id == org_id)
    else:
        base_query = (db.usage_logs.timestamp >= datetime.combine(start_date, datetime.min.time())) & (db.usage_logs.user_id == user_id)
...
def get_usage_by_user():
    ...
    return jsonify({
        'period_days': days,
        'by_user': list(user_usage.values())
    })
...
def get_cost_analytics():
    ...
    records = db(base_query).select(orderby=db.token_usage.date)
