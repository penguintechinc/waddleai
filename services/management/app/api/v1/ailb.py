...
def create_ailb_route():
    ...
    return (
        jsonify({"message": "Use /providers endpoint to create providers, then /providers/{id}/sync to push to AILB"}),
        400,
    )
...
def update_ailb_route(route_id):
    ...
    return jsonify({"route_id": route_id, "message": "Route update not yet implemented. Update provider and re-sync."})
    ...
    return jsonify(
        {
            ...
            "metrics": {
                ...
                "error_rate": 0.02,
                "providers": {
                    "openai": {"requests": 5000, "avg_latency_ms": 180, "success_rate": 0.98},
                    "anthropic": {"requests": 4500, "avg_latency_ms": 140, "success_rate": 0.99},
                    "ollama": {"requests": 3000, "avg_latency_ms": 120, "success_rate": 0.97},
                },
            },
        }
    )
...
def generate_marchproxy_import_config():
    ...
    if download:
        return Response(
            config_str, mimetype=mimetype, headers={"Content-Disposition": f"attachment; filename={filename}"}
        )
