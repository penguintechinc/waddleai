...
DEFAULT_ROUTING_MATRIX: List[Dict[str, Any]] = [
    {'tool_type': 'chat', 'complexity': 'low', 'region': 'us', 'model_name': 'gpt-4o-mini', 'model_params': '8B', 'vram_gb': 8, 'capability_score': 0.7},
    {'tool_type': 'chat', 'complexity': 'medium', 'region': 'us', 'model_name': 'gpt-4o', 'model_params': '200B', 'vram_gb': 40, 'capability_score': 0.9},
    {'tool_type': 'chat', 'complexity': 'high', 'region': 'us', 'model_name': 'claude-3-5-sonnet-latest', 'model_params': '175B', 'vram_gb': 40, 'capability_score': 0.95},
    {'tool_type': 'code', 'complexity': 'low', 'region': 'us', 'model_name': 'codellama', 'model_params': '7B', 'vram_gb': 8, 'capability_score': 0.65},
    {'tool_type': 'code', 'complexity': 'medium', 'region': 'us', 'model_name': 'gpt-4o', 'model_params': '200B', 'vram_gb': 40, 'capability_score': 0.88},
    {'tool_type': 'code', 'complexity': 'high', 'region': 'us', 'model_name': 'claude-3-5-sonnet-latest', 'model_params': '175B', 'vram_gb': 40, 'capability_score': 0.96},
    {'tool_type': 'embed', 'complexity': 'low', 'region': 'us', 'model_name': 'nomic-embed-text', 'model_params': '137M', 'vram_gb': 2, 'capability_score': 0.8},
    {'tool_type': 'embed', 'complexity': 'medium', 'region': 'us', 'model_name': 'text-embedding-3-small', 'model_params': '1.5B', 'vram_gb': 4, 'capability_score': 0.85},
    {'tool_type': 'embed', 'complexity': 'high', 'region': 'us', 'model_name': 'text-embedding-3-large', 'model_params': '3B', 'vram_gb': 8, 'capability_score': 0.92},
    {'tool_type': 'chat', 'complexity': 'low', 'region': 'eu', 'model_name': 'mistral', 'model_params': '7B', 'vram_gb': 8, 'capability_score': 0.7},
    {'tool_type': 'chat', 'complexity': 'medium', 'region': 'eu', 'model_name': 'mixtral', 'model_params': '46.7B', 'vram_gb': 24, 'capability_score': 0.82},
    {'tool_type': 'chat', 'complexity': 'high', 'region': 'eu', 'model_name': 'claude-3-5-sonnet-latest', 'model_params': '175B', 'vram_gb': 40, 'capability_score': 0.95},
]
...
def list_entries() -> tuple:
    ...
    rows = db(query).select(orderby=db.routing_matrix.id)
...
@require_auth
def get_entry(entry_id: int) -> tuple:
...
def create_or_upsert_entry() -> tuple:
    ...
    entry_id: int = db.routing_matrix.insert(
        tool_type=tool_type,
    ...
    new_row = db(db.routing_matrix.id == entry_id).select().first()
    return jsonify({
        'status': 'success',
...
@require_auth
@require_role('admin', 'resource_manager')
def update_entry(entry_id: int) -> tuple:
    ...
    allowed_fields = [
        'tool_type', 'complexity', 'region', 'model_name',
        'model_params', 'vram_gb', 'capability_score', 'enabled',
        'credential_label',
    ]
    update_fields: Dict[str, Any] = {}
...
@require_auth
@require_role('admin', 'resource_manager')
def delete_entry(entry_id: int) -> tuple:
