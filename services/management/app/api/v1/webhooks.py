...
def handle_usage_webhook():
    ...
    return jsonify({
        'status': 'accepted',
        'event_id': event_id,
        'processed': virtual_key is not None
    })
...
def process_usage_event(virtual_key, event_data: dict):
    ...
    waddleai_tokens = calculate_waddleai_tokens(
    ...
    for event_data in events:
        ...
        try:
            ...
            db.ailb_usage_events.insert(
                ...
                timestamp=datetime.fromisoformat(event_data['timestamp'].replace('Z', '+00:00')) if event_data.get('timestamp') else datetime.utcnow(),
                processed=False,
                created_at=datetime.utcnow()
            )
